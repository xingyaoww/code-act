...
from code_actions import PythonREPL
...
PY_FUNC_DESC = [
    ...
    # Only enable a subset following eval_utils.py `action_type_description``
    (
        # "wait() -> str",
        "wait(duration: Optional[int] = None) -> str",
        "Take no action for some duration. wait for something to be done, for example, an object on stove to be boiled"
    ),
    (
        "go_to(location: str) -> str",
        "Move to a new location."
    ),
    (
        "teleport_to(location: str) -> str",
        "Teleport to a specific room. For example, teleport_to(\"kitchen\")"
    ),
    (
        "read(obj: str) -> str",
        "Read an object such as a recipe or a book."
    ),
    (
        "pick_up(obj: str) -> str",
        "Pick up an object and put it into your inventory."
        # "Move an object to the inventory."
    ),
    (
        "open_container(container: str) -> str",
        "Open a container. Open an object with doors before you search or put things in it. For example, open(\"freezer\"), OPEN(\"blast furnace\")."
    ),
    # ( # not in the original list
    #     "close_container(container: str) -> str",
    #     "Close a container."
    # ),
    (
        "activate(device: str) -> str",
        "Activate a device. Activate and turn on an object such as sink or stove, so that you can use it."
    ),
    (
        "deactivate(device: str) -> str",
        "Deactivate a device."
    ),
    (
        "examine(obj: str) -> str",
    ...
    (
        "connect_obj1_to_obj2(obj1: str, obj2: str) -> str",
        "Connect two objects so that they become useful."
    ),
    (
        "move_to(obj: str, container: str) -> str",
        "Move an object to a container/place."
    ),
    (
        "use(item: str, target: Optional[str] = None) -> str",
        "Use a device or item (optionally on a target). For example, use(\"thermometer in inventory\", \"water\") to check the temperature of water."
    ),
    (
        "mix(container: str) -> str",
        "Chemically mix a container. mix the objects in a container such as mix(\"cup containing sugar and water\")"
    ),
    (
        "dunk_into(container: str, liquid: str) -> str",
        "Dunk an object into a liquid."
    ),
    (
        "put_down(obj: str) -> str",
        "Drop an inventory item."
    ),
    (
        "drop(obj_a: str, obj_b: Optional[str] = None) -> str",
        "drop object A into object B (optional)"
    ),
    (
        "pour_into(liquid: str, container: str) -> str",
    ...
    # (
    #     "disconnect_obj(obj: str) -> str",
    #     "Disconnect components."
    # ),
]

PY_FUNC_DESC_STR = ''.join([
    f'[{i+1}] {signature}\n{desc}\n'
    for i, (signature, desc) in enumerate(PY_FUNC_DESC)
])
...
'''.strip() + f"""
You may execute one of the following Python function for your action.

{PY_FUNC_DESC_STR}

For example, you may type "Action: pick_up(\"apple\")" to pick up an apple.
NOTE: You can only execute one function at a time.
"""
...
def map_str_action_to_func(action: str) -> str:
    assert action.startswith("Action: ")
    action = action[len("Action: "):]
    # "Action: open door to kitchen", -> "Action: open(\"door to kitchen\")"
...
def eval(args, task_num, logger):
    ...
    for variation in tqdm(variations, desc="Variations"):
        ...
        recent_actions = ["look_around()"]
        logger.info("Task description: " + task_description)
 
        obs, info = env.reset()
        logger.info("Initial observation: " + obs)
        ...
        conv.append_message(conv.roles[1], 'Action: look_around()')
        conv.append_message(conv.roles[0], clean(obs))
        logger.info("Initial conversation:\n" + str(conv.get_prompt()))
        ...
        while not done:
            ...
            if action.startswith('Think:'):
                ...
                code_action = action.lstrip().split("\n")[0]
                action = code_action_to_str(code_action)
                logger.info(f"Code Action: {code_action}, Execution Output: {action}")
                if action.startswith("Out["):
                    # use regex to extract the output
                    action = re.search(r"Out\[\d+\]: (.*)", action).group(1).strip().strip("'")
                
                if "Traceback" not in action and "Error" not in action:
                    # Get valid actions at this point
                    action = findValidActionNew([action], env, info['look'], recent_actions, None, logger)
                    obs, _reward, done, info = env.step(action)
                    score = info['score']
                else:
                    # If there's an error, make this action a failure
                    obs = action  # the traceback
                    score = 0.0

                if is_action_failed(obs) or "Traceback" in obs or "Error" in obs:
                    fail_counter += 1
                    if fail_counter >= 10:
...
def parse_args():
    ...
    parser.add_argument("--prompt_file", default="prompts/prompt_code_action.json")
